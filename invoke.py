...
async def invoke(args, remaining_args):
    ...
    if run_step in ['all', 'gen_code']:
        if args.llm:  
            ...
            prompt_security_hint = bool(args.prompt_security_hint)
...
def parse_args():
    ...
    codegen_llm_group.add_argument('--retrieval_top_k', type=int, default=None, help='限制上下文检索文件数量（来自候选配置）')
    codegen_llm_group.add_argument('--include_readme', type=int, choices=[0, 1], default=1, help='是否包含README上下文（1/0）')
    codegen_llm_group.add_argument(
        '--prompt_template',
        type=str,
        choices=['default_unified_diff', 'security_minimal_edit', 'security_checklist'],
        default='default_unified_diff',
        help='提示词模板名称',
    )
    codegen_llm_group.add_argument('--prompt_security_hint', type=int, choices=[0, 1], default=0, help='是否启用额外安全提示（1/0）')

    codegen_agent_group = parser.add_argument_group('Agent模式代码生成（指定 --agent 时）选项')
    codegen_agent_group.add_argument('--agent_name', type=str, help='Agent名称')
    ...
    if args.llm:
        ...
        try:
            ...
            parser.error(f"代码生成使用Agent模式时，Agent依赖加载失败：{e}")
        if args.agent_name is None:
            parser.error("代码生成使用Agent模式时，必须指定Agent名称（--agent_name）")
        if args.agent_name not in agent_bench_map:
            parser.error(f"不支持的Agent名称（--agent_name）：{args.agent_name}，可选值：{list(agent_bench_map.keys())}")
